...
-- predzadnjiElement l = head $ reverse $ take 2 $ reverse l
-- predzadnjiElement l = head $ drop 1 $ reverse l 
...
-- [0,0,0,0,0,0]
zbrisi :: Int -> [a] -> [a]
zbrisi k l = prviSeznam ++ drugiSeznam
	where
		prviSeznam = take k l
		drugiSeznam = drop (k+1) l
...
-- [0,0,0,0,0,2,0]
vstavi :: a -> Int -> [a] -> [a]
vstavi x k l = leviDel ++ [x] ++ desniDel
	where
		(leviDel, desniDel) = razdeli k l
...
-- [3,4,5,1,2]
zavrti :: Int -> [a] -> [a]
zavrti n l = leviDel ++ desniDel
	where
		(desniDel, leviDel) = razdeli n l
...
maxPoKomponentah l1 l2 = [max x1 x2 |(x1,x2) <- zip l1 l2]
